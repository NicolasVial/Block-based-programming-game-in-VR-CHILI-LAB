...
#pragma warning restore IDE0044
    ...
    HandAnimation m_handAnimation
    {
        get
        {
            if (_handAnimation == null)
                _handAnimation = GetComponentInChildren<HandAnimation>();
            return _handAnimation;
        }
        set => _handAnimation = value;
    }
    ...
    {
        m_handAnimation = GetComponentInChildren<HandAnimation>();
    ...
    void OnEnterTeleportState(StateId previousStateId)
    {
        // Force the animation to point
        if (m_handAnimation != null)
        {
            m_handAnimation.SetPointing(true);
            m_handAnimation.CanGrab = false;  
        }
        SetTeleportController(true);
    }

    void OnExitTeleportState(StateId nextStateId)
    {
        if (m_handAnimation != null)
        {
            // Stop forcing the animation to point
            m_handAnimation.SetPointing(false);
            m_handAnimation.CanGrab = true;
        }
